...
#idx=1
idx=parse(Int64,ARGS[1]) #read in index defining the ground truth model version: 1=Model2=asynchronous, 2=Model7=multi-frequency, 3=Model16=synchronous
#output_path="./"
output_path=ARGS[2] #read in absolute path to store results in.
...
for j in 1:3
    ...
    name="Model_$(model_names[idx])_$(j)_ground_truth_data"
    println("writing csv")
...
# using Plots
# j=1 #model instantiation number
# gt_read=Matrix(DataFrame(Arrow.Table(output_path*"Model_$(model_names[idx])_$(j)_ground_truth_data.arrow.lz4")))
# plot(gt_read[:,1:5],label="",color=:blue,ylims=(-1.0,1.0))
# plot!(gt_read[:,6:10],label="",color=:red)

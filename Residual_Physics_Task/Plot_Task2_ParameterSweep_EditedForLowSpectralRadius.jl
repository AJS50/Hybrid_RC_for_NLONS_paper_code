...
for arrindex in [1,5,3,4]
    ...
    pvalues_dict=Dict("OmegaError"=>collect(range(0.004,0.08,20)),"OmegaErrorLarge"=>collect(range(0.1,0.48,20)),"KError"=>collect(range(0.004,0.08,20)),"KErrorLarge"=>collect(range(0.1,0.48,20)),"ReservoirSize"=>collect(range(50,1000,20)),"InputScaling"=>collect(range(0.1,2.0,20)),"InputScalingLarge"=>collect(range(1.05,2.0,20)),"SpectralRadiusLow"=>collect(range(0.0,0.08,5)),"SpectralRadius"=>collect(range(0.1,2.0,20)),"MeanDegree"=>collect(range(2,21,20)),"KnowledgeRatio"=>collect(range(0.05,1.0,20)),"Regularisation"=>[0.5,0.25,0.1,0.01,0.001,0.0001,0.00001,0.000001,0.0000001,0.00000001])
    ...
    for (pidx,param) in enumerate(parameters_to_plot)
        ...
        elseif param=="SpectralRadiusLow"
            global arrayindex_range=collect(1:1:5)
        else
        ...
        if param=="SpectralRadius"&&arrindex!=2
            ...
            for (idx,model) in enumerate(models)
                colour=colours[idx]
                collected_mean_errors=Array{Float64,2}(undef, length(arrayindex_range)+5, num_reservoirs)
                for arrayindex in arrayindex_range1
                    if arrindex!=5
                        ...
                        collected_mean_errors[arrayindex-arrayindex_range[1]+6,:]=mean_over_tests 
                    else 
                ...
                for arrayindex in arrayindex_range2
                        norm_errors=Matrix(DataFrame(CSV.read(input_path_base*"SpectralRadiusLow/SpectralRadiusLow"*"_"*model*"_"*"Biharmonic_Kuramoto"*"_"*base_model*"_valid_times_array_index_$(arrayindex).csv",DataFrame)))
                        mean_over_tests=mean(norm_errors[test_num_range,:],dims=1)
                ...
                if arrindex>=2
                    ...
                    elseif pidx==1 
                        plot!(p,vcat(get(pvalues_dict,"SpectralRadiusLow","..")[arrayindex_range2],get(pvalues_dict,param,"..")[arrayindex_range1]),title=model_titles[arrindex],titlefonsize=title_fontsize,mean(collected_mean_errors,dims=2),label=nothing,color=colour,linewidth=3,ribbon=std(collected_mean_errors,dims=2),fillalpha=0.3,legend=false,xlabel=parameters_sweep_names[pidx],ylabel="", xtickfontsize=tickfontsize,ytickfontsize=tickfontsize,legendfontsize=legendfontsize, xlabelfontsize=labelfontsize,ylabelfontsize=labelfontsize,size=(560,480),margin=(5mm),dpi=300);
                    else   
                        plot!(p,vcat(get(pvalues_dict,"SpectralRadiusLow","..")[arrayindex_range2],get(pvalues_dict,param,"..")[arrayindex_range1]),titlefontsize=title_fontsize,mean(collected_mean_errors,dims=2),label=nothing,color=colour,linewidth=3,ribbon=std(collected_mean_errors,dims=2),fillalpha=0.3,legend=false,xlabel=parameters_sweep_names[pidx],ylabel="", xtickfontsize=tickfontsize,ytickfontsize=tickfontsize,legendfontsize=legendfontsize, xlabelfontsize=labelfontsize,ylabelfontsize=labelfontsize,size=(560,480),margin=(5mm),dpi=300);
                    end
                    if param=="Regularisation"
                        plot!(xscale=:log10)
                    end
                    ...
                    if arrindex==4&&pidx==1
                        plot!(p,vcat(get(pvalues_dict,"SpectralRadiusLow","..")[arrayindex_range2],get(pvalues_dict,param,"..")[arrayindex_range1]),titlefontsize=title_fontsize,mean(collected_mean_errors,dims=2),label=model,color=colour,linewidth=3,ribbon=std(collected_mean_errors,dims=2),fillalpha=0.3,legend=:right,xlabel=parameters_sweep_names[pidx],ylabel="Mean t<sup>*</sup> (s)", xtickfontsize=tickfontsize,ytickfontsize=tickfontsize,legendfontsize=legendfontsize, xlabelfontsize=labelfontsize,ylabelfontsize=labelfontsize,size=(560,480),margin=(5mm),dpi=300);
                    elseif pidx==1
                        plot!(p,vcat(get(pvalues_dict,"SpectralRadiusLow","..")[arrayindex_range2],get(pvalues_dict,param,"..")[arrayindex_range1]),title=model_titles[arrindex],titlefontsize=title_fontsize,mean(collected_mean_errors,dims=2),label=nothing,color=colour,linewidth=3,ribbon=std(collected_mean_errors,dims=2),fillalpha=0.3,legend=false,xlabel=parameters_sweep_names[pidx],ylabel="Mean t<sup>*</sup> (s)", xtickfontsize=tickfontsize,ytickfontsize=tickfontsize,legendfontsize=legendfontsize, xlabelfontsize=labelfontsize,ylabelfontsize=labelfontsize,size=(560,480),margin=(5mm),dpi=300);
                    else
                        plot!(p,vcat(get(pvalues_dict,"SpectralRadiusLow","..")[arrayindex_range2],get(pvalues_dict,param,"..")[arrayindex_range1]),titlefontsize=title_fontsize,mean(collected_mean_errors,dims=2),label=nothing,color=colour,linewidth=3,ribbon=std(collected_mean_errors,dims=2),fillalpha=0.3,legend=false,xlabel=parameters_sweep_names[pidx],ylabel="Mean t<sup>*</sup> (s)", xtickfontsize=tickfontsize,ytickfontsize=tickfontsize,legendfontsize=legendfontsize, xlabelfontsize=labelfontsize,ylabelfontsize=labelfontsize,size=(560,480),margin=(5mm),dpi=300);
                    end
                    if param=="Regularisation"
                        plot!(xscale=:log10)
                    end

module KnowledgeReservoirs2

using Distributions, Statistics,  Random, LinearAlgebra, OrdinaryDiffEq, Arrow, CSV, StaticArrays, DataFrames
...
#ext kuramoto callbacks and order funcs:
...
"""
returns default initial conditions for lotka_volterra system [0.1,0.5]. If random=true, then returns initial conditions samples from range[1] to range[2] for each dimension.
"""
...
"""
dynamical system that draws a sin wave, using two dimensions. du[1]=u[2], du[2]=-u[1]

"""
function sin_2d(du,u,p,t)
    du[1]=u[2]
    du[2]=-u[1]
end
...
returns default initial conditions for lotka_volterra system [0.1,0.5]. If random=true, then returns initial conditions samples from range[1] to range[2] for each dimension.
"""
function sin_2d_ic(range::Vector{Float64};random=false)
    if random
        return [range[1]+(range[2]-range[1])*rand(rng) for i in 1:2]
    else
        return [1.0,0.0]
...
function cartesian_biharmonic_kuramoto(du,u,p,t)
    N=Int64((1/2)*size(u,1))
    xs=u[1:N]
    ys=u[N+1:end]
    ωs=p[1:N]
    K=p[N+1]
    ...
    γ_1=p[N+3]
    γ_2=p[N+4]
    ss=[sqrt(x^2+y^2) for x in xs for y in ys]
    for i in 1:N
        du[i]=xs[i]*(ωs[i] + (K/N)*sum([ (cos(γ_1)/sqrt(ss[j]*ss[i]))*(ys[j]*xs[i]-xs[j]*ys[i]) + (sin(γ_1)/sqrt(ss[j]*ss[i]))*(xs[j]*xs[i]+ys[j]*ys[i]) + (a/ss[i]*ss[j])*( (2*cos(γ_2))*(ys[j]*xs[j]*(xs[i]^2-ys[i]^2)-ys[i]*xs[i]*(xs[j]^2-ys[j]^2)) + sin(γ_2)*((xs[j]^2-ys[j]^2)*(xs[i]^2-ys[i]^2) + 4*ys[j]*xs[j]*ys[i]*xs[i])) for j in 1:N]))

        du[i]=-ys[i]*(ωs[i] + (K/N)*sum([ (cos(γ_1)/sqrt(ss[j]*ss[i]))*(ys[j]*xs[i]-xs[j]*ys[i]) + (sin(γ_1)/sqrt(ss[j]*ss[i]))*(xs[j]*xs[i]+ys[j]*ys[i]) + (a/ss[i]*ss[j])*( (2*cos(γ_2))*(ys[j]*xs[j]*(xs[i]^2-ys[i]^2)-ys[i]*xs[i]*(xs[j]^2-ys[j]^2)) + sin(γ_2)*((xs[j]^2-ys[j]^2)*(xs[i]^2-ys[i]^2) + 4*ys[j]*xs[j]*ys[i]*xs[i])) for j in 1:N]))
    end
end
